...
public:  
	...
	Elecciones(int pCandidatos, int pPoblacion, int pTamano) //con parametros
	{
	...
	void votaciones(){    //Metodo que llena el vector con valores al azar
		...
		Sleep(1000);	//para que el copilador se detenga y puedan cambiar los objetos
		int	inferior=35000;		//Limites para aumentar los datos
		int  superior=poblacion-inferior;
		for(int a=0;a<candidatos;a++){
			if(superior>=0){
			votos[a]=inferior + rand() % superior;
	...
	int posmenor(){  //Imprime el candidato con menos votos
		int min=votos[0];
		int posmin=0;
		for(int i=0;i<candidatos;i++)
		{	if(votos[i]<min)
	...
	string abstencionismo(){//devuelve la cantidad y el porcentaje de quienes no votaron
		stringstream s;
		int aux=0;
		for(int i=0; i<candidatos; i++){
			aux=aux+votos[i];
		...
		s<<"No votaron "<<poblacion-emitidos()<<" personas que representan un "<<aux<<" %";
		return s.str();
	}
	...
	string declaracionGanador(){  //devuelve el ganador y sino hay segunda ronda
		...
		if(hayGanador()==true){
			...
			int posmax2=posmenor();
			int max2=votos[posmenor()];
			for(int i=0;i<candidatos;i++)
			{	
				if(i==posmayor()){
					i=i+1;
