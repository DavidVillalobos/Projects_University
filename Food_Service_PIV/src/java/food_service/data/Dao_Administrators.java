...
import food_service.logic.Administrators;
...
public class Dao_Administrators {
    ...
    public void delete(String user_name) throws Exception{
        String sql="DELETE FROM administrators WHERE user_name='%s'";
    ...
    public Administrators get(String user_name) throws Exception{
    ...
    private Administrators render_administrators(ResultSet rs){
        try {
            ...
            p.setAdministratorStatus(Dao_Administrator_Status.instance().get(p.getUserName()));
            return p;
        } catch (Exception ex ) {
            return null;
        }
